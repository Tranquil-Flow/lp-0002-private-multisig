...
| Basecamp app | PARTIAL: native Qt/QML plugin package builds locally, but final runtime load/activation evidence is not attached | Attach `submission/BASECAMP_RUNTIME_LOAD_EVIDENCE.json` with raw-log-bound LogosBasecamp load/activation evidence and include a Basecamp load screenshot/video segment |
