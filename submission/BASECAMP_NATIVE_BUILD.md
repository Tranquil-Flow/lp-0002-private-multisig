...
Honesty note: this proves the native/QML plugin package builds locally against Qt 6 and exposes the Basecamp `IComponent` plugin interface. Runtime launch evidence is attached in `submission/BASECAMP_RUNTIME_LOAD_EVIDENCE.json` with raw runtime logs, log hashes, loaded component id `lp0002_private_multisig`, and `final_basecamp_runtime_load_evidence=true`. Visual component activation remains part of the fresh human-recorded demo video step.
